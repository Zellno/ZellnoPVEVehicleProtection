...
PROJECT = HOME / "dayz/modding/ZellnoPVEVehicleProtection"
BUILD_MOD = PROJECT / "build/@ZellnoPVEVehicleProtection"

SERVER = HOME / "dayz/server"
SERVER_MOD = SERVER / "@ZellnoPVEVehicleProtection"
SERVER_KEY = SERVER / "keys/Zellno.bikey"

CLIENT = HOME / ".local/share/Steam/steamapps/common/DayZ"
CLIENT_MOD = CLIENT / "@ZellnoPVEVehicleProtection"
...
MOD_NAME = "@ZellnoPVEVehicleProtection"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Instala Zellno PVE Vehicle Protection de forma reversível."
        )
    )
    parser.add_argument(
        "--apply",
    ...
    require_file(
        BUILD_MOD / "addons/ZellnoPVEVehicleProtection.pbo"
    )
    require_file(
        BUILD_MOD
        / "addons/ZellnoPVEVehicleProtection.pbo.Zellno.bisign"
    )
    ...
    print("=" * 76)
    print(" Zellno PVE Vehicle Protection — instalação local")
    print("=" * 76)
    print()
    ...
    backup = (
        BACKUP_ROOT
        / f"install-zellno-pve-vehicle-protection-{timestamp}"
    )
    ...
    if server_existed:
        if SERVER_MOD.is_symlink():
            ...
            shutil.copytree(
                SERVER_MOD,
                backup / "@ZellnoPVEVehicleProtection.server",
            )
        else:
            shutil.copy2(
                SERVER_MOD,
    ...
    if client_existed:
        if CLIENT_MOD.is_symlink():
            ...
            shutil.copytree(
                CLIENT_MOD,
                backup / "@ZellnoPVEVehicleProtection.client",
            )
        else:
            shutil.copy2(
                CLIENT_MOD,
    ...
    stage = Path(
        tempfile.mkdtemp(
            prefix=".ZellnoPVEVehicleProtection-stage-",
            dir=SERVER,
        )
    )

    try:
        staged_mod = stage / "@ZellnoPVEVehicleProtection"
        ...
        server_backup = backup / "@ZellnoPVEVehicleProtection.server"
        ...
        client_backup = backup / "@ZellnoPVEVehicleProtection.client"
